...
class ProductComponent extends Component {
    ...
    onChange(event){
        this.checkedIn=event.target.checked;
        this.props.checkedIn(this.checkedIn);
    }
    ...
    handleAmount(k,value){
        ...
        if(currency){
            this.props.accountAmountIn({currency1:currency,currency2:accountCurrency},this.props.form,k);
        }
    }
    handleCurrencySelect(k,value,option){
        ...
        if(amount){
            this.props.accountAmountIn({currency1:value,currency2:accountCurrency},this.props.form,k);
        }else{
        ...
        if(this.incomeIds.length>0){
            confirm({
                ...
                onOk:()=>{
                    ...
                    this.props.accountDaleteIn(keys,this.props.form)
                },
                onCancel(){}
            })
        }
    ...
    render(){
        ...
        let {categoryDataTable,categoryDataTable2} = this.props;
        let formItemLayout = {
            labelCol: {
                xs: { span: 24 },
                sm: { span: 6 },
            ...
            return (
                <Form key={k}>
                    <Row>
                        ...
                        <Col span={1} style={style.SkuMarginTop} >
                ...
                <div style={style.IncomeStyle}>
                    <Row style={{marginBottom:10}}>
                        <Col span="17" offset="1">
                            <span style={{fontSize:16,lineHeight:"50px"}}>收入明细 &nbsp;&nbsp;<Checkbox defaultChecked onChange={this.onChange.bind(this)}/></span>
                        </Col>
                        <Col span="6">
...
const AccountingIncomeForm=Form.create()(ProductComponent);

export default AccountingIncomeForm;